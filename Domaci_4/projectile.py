...
class Projectile:
    ...
    def object_type(self, object, length):
        density = 0.0015
        if object == 'ball':
            area = ((length)**2)*np.pi
            Cd = 0.47
            self.set_parameters(Cd, area, density)
        elif object == 'cube':
            area = (length)**2
            Cd = 1.05
            self.set_parameters(Cd, area, density)
        else:
            print("\n\nInput error. Object can only be 'ball' or 'cube'.\n\n")
                  
    def angle_to_hit_target(self, mass, velocity, constant, area, density, x0, y0, radius, x, y):
        angle1 = []
        angle2 = []
        angle_1 = 0
        angle_2 = 45
        try: 
            while angle_1 >= 0 and angle_1 < 45:
                bum1 = False
                self.reset()
                self.set_initial_conditions(mass, velocity, angle_1, x0, y0)
                self.set_parameters(constant, area, density)
                while self.y[-1] >= 0:
                    self.__move_RK4(0.01)
                    a = self.x[-1] - x
                    b = self.y[-1] - y
                    if a**2 + b**2 <= radius**2:
                        bum1 = True
            ...
            while angle_2 <= 90:
                bum2 = False
                self.reset()
                self.set_initial_conditions(mass, velocity, angle_2, x0, y0)
                self.set_parameters(constant, area, density)
                while self.y[-1] >= 0:
                    self.__move_RK4(0.01)
                    c = self.x[-1] - x
                ...
                self.y = []
                angle_2 += 0.5
            return sum(angle1)/len(angle1), sum(angle2)/len(angle2)
        except ZeroDivisionError:
            print('\n Velocity of the projectile is too low. Increase velocity. \n')
